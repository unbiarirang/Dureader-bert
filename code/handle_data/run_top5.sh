...
#
    ...
    cat $SOURCE_DIR/trainset/search.train.json | python paragraph_extraction_top5.py train \
            > $TARGET_DIR/trainset/search-top5.train.json
    cat $SOURCE_DIR/trainset/zhidao.train.json | python paragraph_extraction_top5.py train \
            > $TARGET_DIR/trainset/zhidao-top5.train.json

    echo "Processing devset"
    cat $SOURCE_DIR/devset/search.dev.json | python paragraph_extraction_top5.py dev \
            > $TARGET_DIR/devset/search-top5.dev.json
    cat $SOURCE_DIR/devset/zhidao.dev.json | python paragraph_extraction_top5.py dev \
            > $TARGET_DIR/devset/zhidao-top5.dev.json
