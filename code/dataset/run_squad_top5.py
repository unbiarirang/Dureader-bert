...
parser.add_argument('--search-input-file', required=False, default='../../data/extracted/trainset/search-fake.train.json')
parser.add_argument('--zhidao-input-file', required=False, default='../../data/extracted/trainset/zhidao-fake.train.json')
#parser.add_argument('--dev-search-input-file', required=False, default='../../data/extracted/devset/search-fake.dev.json')
#parser.add_argument('--dev-zhidao_input_file', required=False, default='../../data/extracted/devset/zhidao-fake.dev.json')
parser.add_argument('--seed', required=False, type=int, default=42)
args = parser.parse_args()
random.seed(args.seed)

TRAINSET_NAME = 'train' + '-top5.data'
DEVSET_NAME = 'dev' + '-top5.data'
...
def read_squad_examples(zhidao_input_file, search_input_file, is_training=True):
    ...
    with open(search_input_file, 'r', encoding='utf-8') as f:
        for line in tqdm(f.readlines()):
            ...
            answer_passage_idx = 0 # only one paragraph
            ...
            doc_tokens = source['documents']['segmented_paragraphs'][answer_passage_idx]
            ques_len = len(source['documents']['segmented_title']) + 1
...
#            for doc_index in range(1, len(source['documents'])):
...
#                yesno_answers = source['yesno_answers']
#            else
#                yesb
            ################
