...
class BertPreTrainedModel(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None, cache_dir=None, from_tf=False, config_name='bert_config.json', *inputs, **kwargs):
        """
...
class BertForQuestionAnswering1(BertPreTrainedModel):

    def __init__(self, config):
        super(BertForQuestionAnswering1, self).__init__(config)
        self.bert = BertModel(config)
...
class BertForQuestionAnswering3(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        ...
        logits = self.dropout_1(sequence_output)
        logits = self.linear(logits)
...
class BertForQuestionAnswering4(BertPreTrainedModel):

    def __init__(self, config):
        super(BertForQuestionAnswering4, self).__init__(config)
        self.bert = BertModel(config)
        self.linear     = nn.Linear(768, 512)
        self.qa_outputs = nn.Linear(512, 2)
        self.loss_fct = CrossEntropyLoss()
...
class BertForQuestionAnswering5(BertPreTrainedModel):

    def __init__(self, config):
        super(BertForQuestionAnswering5, self).__init__(config)
        self.bert = BertModel(config)
        self.dropout    = nn.Dropout(0.1)
        self.linear     = nn.Linear(768, 512)
        self.qa_outputs = nn.Linear(512, 2)
...
class BertForQuestionAnswering6(BertPreTrainedModel):

    def __init__(self, config):
        super(BertForQuestionAnswering6, self).__init__(config)
        self.bert = BertModel(config)
        self.dropout_1  = nn.Dropout(0.1)
        self.linear     = nn.Linear(768, 512)
        self.dropout_2  = nn.Dropout(0.1)
