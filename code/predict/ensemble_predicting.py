...
from modeling import BertForQuestionAnswering, BertForQuestionAnswering1, \
                     BertForQuestionAnswering2, BertForQuestionAnswering3, \
                     BertForQuestionAnswering4, BertForQuestionAnswering5, \
                     BertForQuestionAnswering6, BertConfig

BertForQuestionAnswerings = [BertForQuestionAnswering, BertForQuestionAnswering1, \
                             BertForQuestionAnswering2, BertForQuestionAnswering3, \
                             BertForQuestionAnswering4, BertForQuestionAnswering5, \
...
parser.add_argument('--model-names', required=True, default='[best_model]')
parser.add_argument('--model-nums', required=True, default='[0]')
parser.add_argument('--config-names', required=True, default='[bert_config.json]')
...
model_dir = '../model_dir'
MODEL_PATHS  = [os.path.join(model_dir, model_name) for model_name in json.loads(args.model_names)]
MODEL_NUMS   = json.loads(args.model_nums)
CONFIG_PATHS = [os.path.join(model_dir, config_name) for config_name in json.loads(args.config_names)]
...
def eval_all():
    ...
    evaluate(models, result_file=result_file_path)
